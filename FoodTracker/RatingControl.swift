...
//  FoodTracker
//
//  Created by Adel Amodwala on 2017-07-26.
...
@IBDesignable class RatingControl: UIStackView {
    ...
    func ratingButtonTapped(button: UIButton) {
        ...
        if selectedRating == rating {
            // Reset rating to 0 if the same star was selected
            rating = 0
        } else {
            rating = selectedRating
        }
        
        print("rating now: \(rating)")
    }
    ...
    private func updateButtonSelectionStates() {
        for (index, button) in ratingButtons.enumerated() {
            // If the index of a button is less than the rating, that should be selected
            button.isSelected = index < rating
            
            // Set the hint string for the currently selected start
